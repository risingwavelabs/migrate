...
func init() {
	db := RisingWave{}
	database.Register("risingwave", &db)
}
...
func WithConnection(ctx context.Context, conn *sql.Conn, config *Config) (*RisingWave, error) {
	if config == nil {
	...
	// There's no read-after-write. We need to flush in order to make sure `SetVersion` is visible to subsequent reads.
	if _, err := conn.ExecContext(ctx, "SET RW_IMPLICIT_FLUSH TO true;"); err != nil {
		return nil, err
	}
...
func (p *RisingWave) Open(url string) (database.Driver, error) {
	purl, err := nurl.Parse(url)
	if err != nil {
		return nil, err
	}

	// As RisingWave uses the postgres protocol, and 'postgres' is already a registered database, we need to replace the
	// connect prefix, with the actual protocol, so that the library can differentiate between the implementations
	re := regexp.MustCompile("^(risingwave)")
...
// RisingWve does not support advisory lock functions
func (p *RisingWave) Lock() error {
...
func (p *RisingWave) Unlock() error {
...
func (p *RisingWave) Run(migration io.Reader) error {
...
func (p *RisingWave) runStatement(statement []byte) error {
	ctx := context.Background()
...
func (p *RisingWave) SetVersion(version int, dirty bool) error {
	//tx, err := p.conn.BeginTx(context.Background(), &sql.TxOptions{})
	//if err != nil {
	//	return &database.Error{OrigErr: err, Err: "transaction start failed"}
	//}

	query := `DELETE FROM ` + pq.QuoteIdentifier(p.config.migrationsSchemaName) + `.` + pq.QuoteIdentifier(p.config.migrationsTableName)
	...
	if version >= 0 || (version == database.NilVersion && dirty) {
		...
		if _, err := p.conn.ExecContext(context.Background(), query, version, dirty); err != nil {
			//if errRollback := tx.Rollback(); errRollback != nil {
			//	err = multierror.Append(err, errRollback)
			//}
			return &database.Error{OrigErr: err, Query: []byte(query)}
...
func (p *RisingWave) Version() (version int, dirty bool, err error) {
...
func (p *RisingWave) Drop() (err error) {
...
// convention of "caller locks" in the RisingWave type.
func (p *RisingWave) ensureVersionTable() (err error) {
